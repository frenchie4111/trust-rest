            ...
            if( response_options_body ) {
                ...
                if( !allow_unspecified_keys ) {
                    ...
                    _.each( response_body, function( response_value, response_key ) {
                        const message = `UNKNOWN_KEY - response contains a key not specified in the validation: ${response_key}`;
                        assert.property( response_options_body, response_key, new ValidationError( message ) );
                    } );
                }
                ...
                _.each( response_options_body, function( expected_options, expected_key ) {
                    ...
                    if( !expected_options.hasOwnProperty( 'required' ) || expected_options.required ) {
                        assert.property( response_body, expected_key, new ValidationError( `MISSING_KEY - response does not contain required key: ${expected_key}` ) );
                    }
                    ...
                    if( expected_options.hasOwnProperty( 'value' ) && response_body.hasOwnProperty( expected_key ) ) {
                        const message = `INCORRECT_VALUE - incorrect value for key "${expected_key}" in body. Expected ${response_body[ expected_key ]}, got ${expected_options.value}`;
                        assert.propertyVal( response_body, expected_key, expected_options.value, new ValidationError( message ) );
                    }
                    ...
                    if( expected_options.hasOwnProperty( 'type' ) && response_body.hasOwnProperty( expected_key ) ) {
                        if( !( expected_options.allow_null === true && response_body[ expected_key ] === null ) ) {
                            const message = `INCORRECT_TYPE - expected key "${expected_key}" to be of type ${expected_options.type}`;
                            assert.typeOf( response_body[ expected_key ], expected_options.type, new ValidationError( message ) );
                        }
                    }
                ...
                if( !allow_unspecified_keys ) {
                    ...
                    assert.isUndefined( response_body, new ValidationError( 'NOT_NULL_BODY - expected body to be NULL' ) );
                }
            }
        };

        var _addValidations = function( request, response_options, completion_handler ) {
            if( !completion_handler ) completion_handler = function() {
                console.error( 'trust: Should specify a completion handler' )
            };
            request
                .end( function( err, res ) {
                    ...
                    try {
                        const expected_code = response_options.code || 200;
                        assert.propertyVal( res, 'statusCode', expected_code, `INVALID_HTTP_CODE - expected ${expected_code}, got ${res.statusCode}` );

                        assert.property( res, 'headers', 'MISSING_HEADERS - expected headers in response, got none.' );
                        assert.property( res.headers, 'content-type', 'MISSING_CONTENT_TYPE - expected content-type header in response, got none.' );
                        const message = `INCORRECT_CONTENT_TYPE - expected content-type header of ${response_options.content_type}, got ${res.headers[ 'content-type' ]}`;
                        assert.match( res.headers[ 'content-type' ], response_options.content_type || /json/, message );

                        const expected_type = response_options.type || 'object';
                        assert.typeOf( res.body, expected_type, `INCORRECT_BODY_TYPE - expected body to be type ${expected_type}` );
