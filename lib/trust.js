...
(function() {
    ...
    module.exports = function( url ) {
        ...
        var _validateBody = function( response_body, response_options_body ) {
        ...
        var _addValidations = function( request, response_options, completion_handler ) {
            request
                .expect( response_options.code || 200 )
                .expect( 'Content-Type', response_options.content_type || /json/ )
                .end( function( err, res ) {
                    if( err ) return completion_handler( err );

                    try {
                        _validateBody ( res.body, response_options.body );
                    } catch( err ) {
